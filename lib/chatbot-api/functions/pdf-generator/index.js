...
// The injection/SSRF boundary for user-controlled document content, exported
// so the unit suite (test/lambdas/pdf-generator) can exercise it directly.
exports.escapeHtml = escapeHtml;
exports.processContent = processContent;
exports.isAllowedFontRequest = isAllowedFontRequest;
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Per-language chrome for each language section, matching the app UI
    // wording (summary.iepSummary / summary.keyInsights translation keys).
    // {pages} / {language} are interpolated below.
    const PDF_LABELS = {
      en: { translation: 'Translation', iepSummary: 'IEP Summary', keyInsights: 'Key Insights', reference: 'Reference: Pages {pages} of original IEP document', noContent: 'No content available in {language}', coverTitle: 'IEP Document Summary and Translations', generatedOn: 'Generated on {date}' },
      es: { translation: 'Traducción', iepSummary: 'Resumen del IEP', keyInsights: 'Información Clave', reference: 'Referencia: Páginas {pages} del documento IEP original', noContent: 'No hay contenido disponible en {language}', coverTitle: 'Resumen y traducciones del documento IEP', generatedOn: 'Generado el {date}' },
      zh: { translation: '翻译', iepSummary: 'IEP摘要', keyInsights: '关键信息', reference: '参考：原 IEP 文件第 {pages} 页', noContent: '没有{language}的可用内容', coverTitle: 'IEP 文件摘要与翻译', generatedOn: '生成日期：{date}' },
      vi: { translation: 'Bản dịch', iepSummary: 'Tóm tắt IEP', keyInsights: 'Thông tin chính', reference: 'Tham khảo: Trang {pages} của tài liệu IEP gốc', noContent: 'Không có nội dung bằng {language}', coverTitle: 'Tóm tắt và bản dịch tài liệu IEP', generatedOn: 'Tạo ngày {date}' },
      ar: { translation: 'الترجمة', iepSummary: 'ملخص IEP', keyInsights: 'أبرز النقاط', reference: 'المرجع: الصفحات {pages} من مستند IEP الأصلي', noContent: 'لا يوجد محتوى متاح باللغة {language}', coverTitle: 'ملخص مستند IEP وترجماته', generatedOn: 'تاريخ الإنشاء: {date}' },
    };
    const getLabels = (language) => PDF_LABELS[language] || PDF_LABELS.en;

    // Generate HTML content for the PDF
    const generateHTMLContent = () => {
      let htmlContent = '';
      
      // Document header: English always, plus the preferred language when it
      // isn't English (the PDF always pairs a translation with English, so the
      // cover mirrors that)
      const coverLang = (preferredLanguage && preferredLanguage !== 'en' && PDF_LABELS[preferredLanguage])
        ? preferredLanguage
        : null;
      const coverDir = coverLang === 'ar' ? 'rtl' : 'ltr';
      htmlContent += `
        <div class="document-header">
          <h1>IEP Document Summary and Translations</h1>
          ${coverLang ? `<h1 lang="${escapeHtml(coverLang)}" dir="${coverDir}">${PDF_LABELS[coverLang].coverTitle}</h1>` : ''}
          <p class="subtitle">Generated on ${new Date().toLocaleDateString()}</p>
          ${coverLang ? `<p class="subtitle" lang="${escapeHtml(coverLang)}" dir="${coverDir}">${PDF_LABELS[coverLang].generatedOn.replace('{date}', new Date().toLocaleDateString(coverLang))}</p>` : ''}
        </div>
      `;
      ...
      orderedLanguages.forEach((language) => {
        ...
        // Languages with different directions coexist in one PDF, so direction
        // is set per language section rather than on <html>.
        const sectionDir = language === 'ar' ? 'rtl' : 'ltr';

        htmlContent += `
          <div class="language-section" lang="${safeLang}" dir="${sectionDir}">
            <h2 class="language-header" lang="${safeLang}">
              ${isTranslation ? `${labels.translation} - ` : ''}${safeLanguageName}
            </h2>
        `;
        ...
        if (!languageSections || languageSections.length === 0) {
          if (!summary || !summary.trim()) {
            htmlContent += `
              <p class="empty-content">
                ${labels.noContent.replace('{language}', safeLanguageName)}
              </p>
            `;
          }
        } else {
          // Add sections header
          htmlContent += `<h3>${labels.keyInsights}</h3>`;
          ...
          languageSections.forEach((section) => {
            ...
            const safePageReference = Array.isArray(section.pageNumbers) && section.pageNumbers.length > 0
              ? `<p class="page-reference">${labels.reference.replace('{pages}', escapeHtml(section.pageNumbers.join(', ')))}</p>`
              : '';
    ...
    const htmlDocument = `
      ...
      <head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@400;700&family=Noto+Sans:wght@400;700&family=Noto+Naskh+Arabic:wght@400;700&display=swap" rel="stylesheet">
        <style>
          * {
          ...
          *:lang(vi) {
          ...
          /* Arabic: Naskh style for long-form reading; letter-spacing must stay
             0 or cursive letter joining breaks */
          *:lang(ar) {
