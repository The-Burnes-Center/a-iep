 ...
 * clientMetadata.language -> 'locale' user attribute -> user profile
 * (DynamoDB) -> 'en'.
 *
 * The 'locale' attribute matters because Cognito does NOT forward
 * InitiateAuth clientMetadata to the CreateAuthChallenge / CustomMessage
 * triggers (only RespondToAuthChallenge metadata reaches them), so the
 * first login SMS can only be localized from user attributes or the
 * stored profile. The profile lookup is best-effort: any failure falls
 * back to English so authentication is never blocked by localization.
 */
async function resolveLanguage(event) {
    ...
    try {
        ...
        console.warn(`Language lookup failed for user ${userId}, defaulting to English:`, error.message);
    }
