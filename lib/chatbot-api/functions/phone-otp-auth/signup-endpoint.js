...
/**
 * Staging-only E2E backdoor for the bot check, in the shape the OTP backdoor
 * already uses.
 *
 * Turnstile refuses automated browsers. That is the entire product, and it
 * means the real widget and an automated signup cannot both work. Losing the
 * signup journey is the worse trade: it is the coverage that caught a phone
 * signup bug which had been broken for over a month. So staging keeps the REAL
 * widget for anyone testing by hand, and only the E2E runner has a way past
 * it.
 *
 ...
 *   2. The number must be one of the NANP-fictional test numbers, the same
 *      block the OTP backdoor is limited to. A leaked token is therefore
 *      useless for creating an account anyone could receive a text on.
 *
 * The rate limits above still apply: this skips the bot check, nothing else.
 * Compared in constant time, because a byte-by-byte comparison on a shared
 * token is a free oracle.
 */
let bypassToken;
async function isE2EBypass(token, phoneNumber) {
    const parameterName = process.env.E2E_BYPASS_PARAM;
    if (!parameterName || typeof token !== 'string' || token.length === 0) {
        return false;
    }
    const testPrefixes = (process.env.TEST_PHONE_NUMBERS || '')
        .split(',').map((p) => p.trim()).filter(Boolean);
    if (!testPrefixes.includes(phoneNumber)) {
        return false;
    }
    ...
    console.log('SIGNUP_E2E_BYPASS the bot check was skipped for a test number');
    return true;
}
...
exports.handler = async (event) => {
    ...
    if (secret && !(await isE2EBypass(body.turnstileToken, phoneNumber))) {
