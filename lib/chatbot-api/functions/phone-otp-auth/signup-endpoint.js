...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Immediately, and in the same function. AdminCreateUser leaves the
        // account in FORCE_CHANGE_PASSWORD with a password the caller could
        // otherwise be told; this replaces it with one nobody knows and
        // moves the account to CONFIRMED so custom-auth sign-in works.
        ...
        if (error.name === 'UsernameExistsException') {
            ...
            // sign in. Saying so would also confirm the number is registered,
            // which is the enumeration PreventUserExistenceErrors avoids.
            console.log('Signup for an existing account; the caller proceeds to sign-in');
            return respond(200, { created: false });
        }
