...
const { isE2EBypass: sharedE2EBypass } = require('./e2e-bypass');
...
/**
 * Staging-only E2E backdoor for the bot check.
 *
 * The gate itself now lives in e2e-bypass.js, because /auth/start needs the
 * same one for email destinations and two copies of a security check drift.
 * What stays here is this endpoint's own second lock: the number must be one
 * of the NANP-fictional test numbers, so a leaked token is useless for
 * creating an account anyone could receive a text on. That check is passed in
 * rather than looked up in the shared module, so each endpoint keeps its own.
 */
const isE2EBypass = (token, phoneNumber) => sharedE2EBypass(
    token,
