 ...
 * - User profile creation for new users, recording the identity that was
 *   actually verified rather than assuming a phone number
    ...
    try {
        ...
        if (isValid) {
            ...
            try {
                ...
                await createUserProfileIfNotExists(userId, uiLanguage, event.request.userAttributes);
            } catch (error) {
...
/**
 * What the completed OTP round actually proves, as profile fields.
 *
 * Read from request.userAttributes, which AWS documents as the user's standard
 * attributes and is the only authoritative identity signal this trigger gets.
 * Deliberately NOT parsed out of event.userName: AWS documents that only as
 * "the current user's username", so its format is a local convention
 * (signup-endpoint.js happens to pass the E.164 number as Username) and not a
 * contract we may lean on.
 *
 * Phone wins when both attributes exist, because create-auth-challenge reads
 * phone_number first and texts the code, so SMS is the channel possession was
 * proved on. Neither attribute means we cannot say what was proved, so the
 * profile asserts nothing rather than guessing: this writes a durable record.
 */
function identityFields(userAttributes = {}) {
    const phone = (userAttributes.phone_number || '').trim();
    if (phone) {
        return { authMethod: 'phone', phoneVerified: true };
    }

    const email = (userAttributes.email || '').trim();
    if (email) {
        return { authMethod: 'email', emailVerified: true };
    }
 ...
 */
async function createUserProfileIfNotExists(userId, uiLanguage, userAttributes) {
    ...
    try {
        ...
        // What this sign-in proved, and about which identity. Absent rather
        // than guessed when the attributes say neither.
        const identity = identityFields(userAttributes);
        if (!identity) {
            console.warn(`No phone_number or email on ${userId}; profile records no verified identity`);
        }

        // Create default profile for an OTP-verified user
        const newProfile = {
            userId: userId,
            createdAt: currentTime,
            ...
            showOnboarding: true,
            // authMethod plus the matching verified flag: phone for an SMS
            // OTP, email for an emailed one. See identityFields.
            ...identity,
        ...
        console.log(`Created default profile for ${identity?.authMethod || 'unidentified'} user ${userId}`);
