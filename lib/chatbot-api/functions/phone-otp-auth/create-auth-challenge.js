...
const { E164, FICTIONAL_TEST_EMAIL } = require('./destination');
...
/**
 * Refuse one destination, and record why.
 *
 * Logged rather than silent, so a refusal is diagnosable: an unlogged
 * validation rejection is a failure nobody can explain afterwards.
 * SMS_REFUSED_DESTINATION is a stable marker, not prose: a metric filter
 * alarms on it, and rewording a sentence must not silently disarm an alarm.
 * Same contract as RECORD_FAILURE in the pipeline. `detail` carries at most
 * the dialling prefix; the sanitize.js discipline applies to the rest of the
 * number, which never reaches CloudWatch.
 *
 * Every refusal answers the caller with the SAME sentence whatever the
 * reason. Which reason it was is a log line, not a response field: nothing a
 * caller can see separates a destination this service does not serve from one
 * it could not read, or from an account that is not there at all.
 */
function refusedDestination(reason, detail) {
    console.error(`SMS_REFUSED_DESTINATION reason=${reason} ${detail}`);
 ...
 *
 * The number reaching here has already passed the shape gate in
 * validatePhoneNumberFormat, so this compares a real E.164 value against the
 * configured prefixes and nothing else.
 */
function enforceAllowedDestination(phoneNumber, prefixes) {
    ...
    throw refusedDestination(
        'country-code',
        `prefix=${phoneNumber.slice(0, 4)} allowed=${prefixes.join(',')}`
    );
}
...
/**
 * Is this stored value a phone number at all?
 *
 * The expression comes from destination.js, which is what /auth/start already
 * vets a typed destination with, rather than a second one written here: the
 * front door and the send path have to agree about what a real number is, and
 * when two expressions disagree it is the looser one that decides what reaches
 * the carrier. This function used to carry its own, and it accepted values the
 * front door would have turned away.
 *
 * Fails CLOSED, and deliberately does not repair anything. A value missing its
 * leading '+', or carrying stray whitespace, is refused rather than coerced
 * into the number it probably meant: guessing a destination is how a code goes
 * somewhere nobody asked for it to go. The refusal is logged with its reason,
 * so an account with an unusable attribute is diagnosable instead of looking
 * like broken delivery.
 *
 * Runs ahead of the allowlist, and ahead of the language handshake, so a
 * destination nothing can be sent to is settled before any of the work.
 */
function validatePhoneNumberFormat(phoneNumber) {
    if (!E164.test(phoneNumber)) {
        // Nothing about the value itself: if it is not a phone number, there
        // is no prefix worth recording and no safe way to quote the rest.
        throw refusedDestination('unreadable', 'shape=not-e164');
    }
...
async function sendSMS(phoneNumber, otpCode, language = 'en') {
    // replaceAll, not replace: a template may name a placeholder more than
    // once (the email says {minutes} in both the preview line and the body),
    // and replace would fill the first and leave the rest reading literally
    // "{minutes}" in a message a parent gets.
    const message = getMessages(language).otpLoginSms
        .replaceAll('{code}', otpCode)
        .replaceAll('{minutes}', OTP_EXPIRY_MINUTES);
...
async function sendEmail(address, otpCode, language = 'en') {
    ...
    // replaceAll, not replace: {minutes} appears twice in the email (the
    // preview line and the body), and replace would leave the second one
    // reading literally "{minutes}". See sendSMS.
