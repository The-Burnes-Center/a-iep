...
// Global SMS budget, counted across every phone number. MAX_SMS_PER_HOUR
// bounds one victim's inbox and cannot bound spend, because a pumping run
// never texts the same number twice; this is the control that bounds spend.
//
// Sized so OUR ceiling binds before the account's SNS monthly spend cap does.
// At US rates (~$0.006/message) 200/day is ~$36/month against a $50 cap. That
// ordering is the point: when the SNS cap is reached, Publish does NOT throw.
// It returns a MessageId and the message is silently dropped, so the handler
// logs "SMS sent successfully", the UI tells a parent a code is on its way,
// and no code ever arrives. That is exactly what happened on 2026-09-09.
// Refusing here instead produces a real error the parent can act on.
const MAX_SMS_PER_HOUR_GLOBAL = 100;
const MAX_SMS_PER_DAY_GLOBAL = 200;

// SNS refuses to send when a message would cost more than this. US SMS is
// ~$0.006 and Canadian ~$0.021, both +1 and both well under; the premium
// international routes that SMS pumping monetizes are not. A second,
// independent lock on the destination allowlist: it holds even if the
// allowlist is ever misconfigured. Was '0.50', which permitted essentially
// every destination on earth and so bounded nothing.
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!secretLoginCode) {
            ...
            // so it draws on no budget. Global before per-phone so a run that
            // never repeats a number still trips something.
            if (!testNumber) {
                await enforceGlobalSmsBudget();
 ...
 *
 * Deliberately fails CLOSED, unlike enforceSmsRateLimit below, on a missing
 * table as well as on a DynamoDB error. The asymmetry is the whole lesson of
 * 2026-09-09: a control that fails open stops working exactly when it is
 * needed, and the two outages are not comparable. Refusing to text during a
 * DynamoDB outage costs minutes, and costs nothing extra in practice because
 * DynamoDB already holds the profiles and documents this app is built on, so
 * it is down anyway. Draining the SNS budget costs every family their login
 * until the calendar month rolls over.
 *
 * Counters are separate rows from the per-phone ones (pk 'GLOBAL#H#<hour>'
 * and 'GLOBAL#D#<day>') on the same table and TTL, so no schema change.
 */
async function enforceGlobalSmsBudget() {
    ...
    const windows = [
        { pk: `GLOBAL#H#${Math.floor(Date.now() / hourMs)}`, max: numericEnv('MAX_SMS_PER_HOUR_GLOBAL', MAX_SMS_PER_HOUR_GLOBAL), ttlSeconds: 2 * 60 * 60, label: 'hourly' },
        { pk: `GLOBAL#D#${Math.floor(Date.now() / (24 * hourMs))}`, max: numericEnv('MAX_SMS_PER_DAY_GLOBAL', MAX_SMS_PER_DAY_GLOBAL), ttlSeconds: 48 * 60 * 60, label: 'daily' },
    ];
    ...
    for (const { pk, max, ttlSeconds, label } of windows) {
        ...
        if (count > max) {
            // Loud on purpose: this is the alarm-worthy signal that either an
            // abuse run is under way or the service has outgrown the ceiling.
...
/** Read a positive integer override, ignoring anything unparseable. */
