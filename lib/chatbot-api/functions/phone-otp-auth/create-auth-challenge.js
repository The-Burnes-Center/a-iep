...
// bound total spend.
//
// Deliberately sized to bind before the provider's own account ceiling,
// because the provider accepts and drops a message rather than failing the
// call once that ceiling is reached. Refusing here yields a real error the
// caller can act on instead of a message that silently never arrives.
//
// Operational values come from the environment; see the deployment config.
